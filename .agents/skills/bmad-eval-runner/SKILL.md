...
1. Resolve config the same way `bmad-workflow-builder` does (`{project-root}/_bmad/bmadconfig.yaml` then `config.user.yaml`, falling back to `bmb/bmadconfig.yaml`). Resolve `{user_name}`, `{communication_language}`, `{bmad_builder_reports}`. Apply throughout the session.
